...
## 열차가 후진하는 현상 발생
 - 예측을 통한 위치 보정 실패로 추정
 - `지금 쯤이면 여기에 있겠지`라고 예측하고 위치를 바꾸었으나 아직 열차는 그곳에 도착하지 않았고, 위치를 바꾼 뒤에 열차의 정확한 위치가 수집된 것으로 추정
 - 열차가 특정 역에 도착할 시간이 되면, 열차가 아직 역으로 접근 중이여도 도착한 것으로 표시. 이후에 도착에서 접근으로 변경됨.
