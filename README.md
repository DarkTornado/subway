...
### 일본 도쿄 지하철 - 도쿄도 교통국에서 운영 (도영(도에이) 지하철)
 - 아사쿠사선, 미타선, 신주쿠선, 오에도선 (1, 6, 10, 12호선)
 - 도쿄 사쿠라 트램 (아라카와선) (노면전차. 지하철 아님)
 - 닛포리·토네리 라이너 (경전철. 지하철 아님)

### 일본 도쿄 지하철 - 도쿄메트로에서 운영
 - 히비야선, 긴자선, 마루노우치선, 도자이선 (2 ~ 5호선)
...
## 지원 중단 예정 노선
 ...
 - [ ] JR 큐슈 노선 전체
 - [ ] 후쿠오카 지하철 (어차피 [별도 레포](https://github.com/DarkTornado/FukuokaCitySubway)가 존재)
 - [ ] 타오위안 지하철 (공항철도)
...
## 열차 증발 버그
 - 열차 운행 상태가 마지막으로 갱신된지 약 5분이 지나면 사라짐
 - 기점에서 출발 대기중인 열차, 급행열차 추월을 기다리는 완행열차, 다음 역까지 이동하는데 걸리는 시간이 5분을 넘는 구간에 있는 열차 등이 사라짐
...
## 급행 열차 순간이동 버그
 - `급행 열차가 정차하는 역`에 도착해야 급행 열차 위치가 갱신됨
 - e.g. `주안역`을 출발한 `동인천급행`이 `도원역`에 도착해도 여전히 `주안역`에 있다고 표시되며, `제물포역`에 진입해야 열차 위치 갱신
 - `주안역`과 `제물포역`은 `급행 열차가 정차하는 역`, `도원역`은 두 역 사이에 있는 `급행 열차가 동과하는 역`.
...
## 열차 출발 미반영 버그
 - 한국철도공사 관할 구간에서는 열차가 출발해도 여전히 도착으로 표기되는 현상
 - 한국철도공사와 서울시의 열차 출발 상태 처리 방식이 상이한 것이 원인이며, 관련 민원이 접수된지 2년째 해결되지 않음
...
## 4호선 진접선 구간 정보 없음 (불암산 ~ 진접)
 - 불암산(구 당고개)역 진입 이후에는 진접행 열차의 정보가 제공되지 않음
 - 진접선 구간은 남양주시의 협조가 있어야 실시간 운행 정보를 제공할 수 있다고 답변했는데, 정작 본인들이 만든 앱에서는 진접선 구간은 아주 잘 나옴.
 - 진접선 구간과 `8호선 별내선 (별내 ~ 다산) 구간`은 같은 곳에서 관리하는데, 별내선 구간은 아주 잘 나옴.
 - 서울교통공사 홈페이지에도 진접선 구간이 잘 나오고 있기에, 크롤링으로 보정.
 - 서울시 API와 서울교통공사 홈페이지는 다른 전산

## 4호선 진접행 열차들이 전부 불암산행으로 표시되는 현상
 - 서울교통공사 홈페이지 크롤링으로 행선지 보정

## 공항철도 열차 증발 버그
 - 종종 특정 구간에서 일부 열차가 사라졌다가 다시 나타나는 현상 발생
 - 데이터 원천에서는 해당 현상이 발생하지 않음.
 - 위에서 언급한 `열차 증발 버그`에서 `다음 역까지 이동하는데 걸리는 시간이 5분을 넘는 구간에 있는 열차`에 해당하는 것으로 추정
...
## 신림선 행선지 정보가 잘못됨 - 2024년 8월 9일에 해결된 것 확인
 - `샛강행`과 `관악산행`이 `소요산행`과 `병점행`으로 표시됨
...
# 네이버 지도 부산 지하철 실시간 열차 위치특이사항

## 열차가 후진하는 현상 발생
 - 예측을 통한 위치 보정 실패로 추정
 - 열차가 해당 역에 도착할 시간이 되면, 열차가 아직 역으로 접근 중이여도 도착한 것으로 표시. 이후에 도착에서 접근으로 변경됨.

## 종점 도착시 열차 증발
 - 열차가 종점에 도착하면 `operatingStatus`가 'END`가 되면서 `네이버 지도`에는 열차가 뜨지 않음
 - `operatingStatus`가 `END`여도 1분 동안은 종점 도착으로 뜨도록 구현 예정
